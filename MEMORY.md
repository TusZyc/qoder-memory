...
## EVE ESI 后台管理（2026-03-18 规划，2026-03-20 实现）
...
### 已实现功能（2026-03-20）
- **后台首页**：系统状态监控、最近登录用户、API 调用统计
- **日志查看**：Laravel 日志实时查看、级别过滤、关键词搜索
- **缓存管理**：分类统计、TTL 分布、内存占用、键详情、分类清除
- **数据管理**：JSON 数据文件状态、新鲜度监控、数据预览
- **API 统计**：调用时间分布图、模块统计、错误/警告详情

### 关键技术点
- 日志解析：正则表达式使用非捕获组 `(?:)` 避免索引偏移
- 侧边栏布局：`flex-shrink: 0` + 固定宽度防止内容区挤压
- 路由：`/admin/*`，中间件 `AdminAuth` 检查角色 ID
