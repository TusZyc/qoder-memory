...
| 任务 | 状态 | 优先级 | 负责 |
|------|------|--------|------|
| **KB 新版接口适配 / KM 详情恢复** | ✅ 已完成：搜索兼容新版 protobuf，详情通过 KB WASM 门禁获取 hash 后走官方 ESI，完整 KM 已恢复 | 最高 | [Codex] |
...
| 日期 | 内容 | commit | 操作者 |
|------|------|--------|--------|
| 4-26 | KB 更新后接口适配：新版 protobuf headers/cookies、autocomplete、groups/regions/next 参数；移除前端直连 beta 详情；后端统一接管详情获取 | `4de8213` | [Codex] |
| 4-26 | 复用 KB 官方风格 WASM glue 打通 `el_psy_congroo -> find_gate_async -> Steins;Gate -> kill/info -> hash -> ESI`，新增 Node hash resolver 并接入 Laravel | `9e380b3` | [Codex] |
| 4-26 | 修复 PHP-FPM 下 `proc_close() === -1` 误判脚本失败，服务器已恢复完整 KM 详情接口 | `261f012` | [Codex] |
...
| 项目 | 值 |
|------|-----|
| 最后成功部署 | 2026-04-26 [Codex via SSH + Docker] |
| 服务器当前 commit | 代码已部署 KB hash 门禁修复；与 GitHub 同步后以最新主分支为准 |
...
12. **KB 新版详情门禁**：`beta.ceve-market.org/app/kill/{id}/info` 等详情接口需要先完成 WASM PoW；普通 cookie + `FinalFantasy-XIV` header 不足以访问，会返回 403。
13. **KM hash 获取现状**：已确认 `api认证` 链接是前端用 `km.Hash` 拼接，静态 HTML 不含 hash；当前项目通过服务端复用 KB WASM 流程获取 hash，再走官方 ESI 拿完整详情。
14. **PHP-FPM 子进程坑**：Node 脚本在容器里可成功输出 hash，但 `proc_close()` 在 FPM 场景可能返回 `-1`；只要 stdout 已解析出合法 hash，应直接视为成功并缓存。
