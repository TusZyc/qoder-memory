...
**优先级**: 最高
**负责**: [Codex]
**目标**: 先做稳定可用的基础版，再继续复杂属性计算

### 本轮已完成
- [x] 决定不再继续堆补丁，改为阶段化重写
- [x] 保留 `/fitting-simulator` 页面入口和现有数据接口方向
- [x] 重建新版基础页面骨架
- [x] 页面主流程改成“先点槽位，再点装备”
- [x] 先保留资源检查与基础属性展示，移除临时拼装的假联动思路
- [x] 修正舰船分类名映射，替换“跳跃货舰”为“战略货舰”
- [x] 装备槽位判断优先改用本地中文分类，减少高/中/低槽分错
- [x] 装备列表按中文子分类分组显示
- [x] 模块图片地址改为物品图，不再统一走舰船渲染图
...
### 说明
- 当前方向是“基础可用优先”，不是一次做完整 Dogma 计算
- 复杂属性实时变化、EHP、DPS、电容等留到后续阶段
...
### 2026-04-10 Codex 补充
- [x] 修复装配模拟器新分类树上线后出现的 500/超时问题，改为“本地分类为主，按需取详情”
- [x] 舰船分类树切到本地中文分类路径，去掉顶层“舰船”，更接近游戏内查找方式
- [x] 装备分类树补上槽位过滤，减少高/中/低槽分类串位
- [x] 图片改为直接走 `image.evepc.163.com/Type/...`，不再走服务器中转
- [x] 舰船树和装备树改为“最后一级分类下直接显示具体物品”，不再统一掉到最底部
- [x] 装配区舰船图改为优先走 `Render`
- [x] 装备 `Type` 图限制在 `64`，避免装配区安装后因请求 `128` 而缺图
- [ ] 等 Tus 按游戏经验继续校准舰船与装备分类细节，确认哪些分类名称和层级仍需人工修正
- [ ] 继续调研“游戏内装备分类树”后，再决定装配模拟器左侧装备树是否要完全对齐市场样式
