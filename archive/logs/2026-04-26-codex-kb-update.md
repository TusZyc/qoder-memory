...
后续完成情况：
- 已改为复用 KB 官方风格 glue，成功跑通 `el_psy_congroo -> find_gate_async -> Steins;Gate -> kill/info -> km.Hash -> ESI`。
- 已新增：
  - `scripts/kb_gate_hash.mjs`
  - `app/Services/Killmail/KbGateHashResolver.php`
  - `config/killmail.php`
- 已接入 `KillmailService`，详情获取时会自动尝试回填 `kb:esi_hash:{killId}` 缓存。

## 最终修复与线上结果

- 本地和容器内均验证 `node scripts/kb_gate_hash.mjs 22490593` 可返回真实 hash：
  - `4be4200fd98eef2997552d2a48a95c3fc137f9fe`
- 服务器初次接入后又踩到一层 PHP-FPM 子进程问题：
  - Node 脚本实际已成功输出 JSON
  - 但 `proc_close()` 返回 `-1`
  - 导致 Laravel 误判为脚本失败
- 最终修复方式：
  - `KbGateHashResolver` 改为优先解析 stdout
  - 只要已经拿到合法 40 位 hash，就直接视为成功并缓存
  - `proc_close` 仅保留为辅助日志

最终线上验证：
- `/api/killmails/advanced-search?...` 正常
- `/api/killmails/kill/22490593` 返回 `success: true`
- `esi_verified: true`
- 已返回完整 `attackers`、`items`、受害者装备和掉落信息
